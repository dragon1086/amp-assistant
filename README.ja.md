...
盲検A/B評価: amp ON vs 単独GPT-5.2。Geminiをジャッジとして使用（モデルIDをランダム化）。N=30問、7ドメイン。

| ドメイン | ampが勝利 | 単独が勝利 | amp勝率 |
|---------|:--------:|:---------:|:------:|
| リソース配分 | 4 | 1 | **80%** |
| 戦略 | 4 | 2 | **67%** |
| 感情 | 3 | 2 | 60% |
...
**正直な解釈:** ampが常に優れているわけではありません。多視点が有効な複雑な戦略・リソース配分問題で大きく優位に立ちます。事実的なQ&Aや単純なアドバイスには単独モデルで十分です。

---

## 先行研究との比較

| プロジェクト | 出典 | 目的 | pip | KGメモリ | CSER | エージェント独立 | MCP |
|------------|------|------|:---:|:---:|:---:|:---:|:---:|
| **amp** | OSS | ライフ意思決定アドバイザリ | ✅ | ✅ | ✅ | ✅ | ✅ |
| llm_multiagent_debate | ICML 2024 | 数学/MMLU精度 | ❌ | ❌ | ❌ | ❌ | ❌ |
| DebateLLM | InstaDeep 2024 | 医療Q&Aベンチマーク | ❌ | ❌ | ❌ | ❌ | ❌ |
| AutoGen | Microsoft | タスク自動化 | ✅ | ❌ | ❌ | ❌ | ❌ |
| CrewAI | 商用 | エンタープライズワークフロー | ✅ | ❌ | ❌ | ❌ | ❌ |

**核心の違い:** 学術的MAD論文はMMLU/数学の正確度向上のために設計されました。ampはオープンエンドな意思決定品質のために設計されています。

---

## 内部構造

技術詳細: [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)

### 知識グラフ

```
ストレージ : SQLite (~/.amp/kg.db) — 単一ファイル、サーバー不要
埋め込み   : OpenAI text-embedding-3-small（1536次元）
検索       : numpy コサイン類似度 — O(n)、~10万ノードまで適用可
```
...
# unique_a = Aのみが提示したアイデア
# unique_b = Bのみが提示したアイデア
# CSER ≥ 0.30 → 合成進行 | CSER < 0.30 → 4ラウンド自動昇格
```

### 動的ドメインレジストリ

9つの静的ドメイン（キャリア、投資など）でカバーできないクエリは、LLMが新しいドメインを自動生成してSQLiteに保存します。`amp domains`で学習済みドメイン一覧を確認できます。
