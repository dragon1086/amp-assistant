...
GPT-5 + GPT-5를 다른 프롬프트로 쓰면 어휘 중복이 높습니다. 훈련 데이터, RLHF 정렬, 사전 분포가 공유됩니다 — 다양성은 대부분 표면적입니다.
...
9개 기본 제공 도메인 (+ 동적 도메인 무제한 자동 생성). 교차 벤더 쌍(GPT × Claude)은 이 처리를 건너뜁니다 — 다양성이 구조적이기 때문입니다.
